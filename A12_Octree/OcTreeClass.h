...
class OcTreeClass
{
	...
	OcTreeClass* m_pParent = nullptr;// Will store the parent of current octant
	OcTreeClass* m_pChild[8];//Will store the children of the current octant

	std::vector<uint> m_lBOs; //stores all the BOs under this octant
	OcTreeClass* m_pRoot = nullptr;//Bounding Object Manager Singleton
	std::vector<OcTreeClass*> m_lChild; //list of nodes that contain objects
...
public:
	...
	OUTPUT: class object
	*/
	OcTreeClass(uint a_nMaxLevel = 3, uint a_nIdealBOCount = 5);
	/*
	USAGE: Constructor
	ARGUMENTS:
	...
	OUTPUT: class object
	*/
	OcTreeClass(vector3 a_v3Center, float a_fSize);
	/*
	USAGE: Copy Constructor
	ARGUMENTS: class object to copy
	OUTPUT: class object instance
	*/
	OcTreeClass(OcTreeClass const& other);
	/*
	USAGE: Copy Assignment Operator
	ARGUMENTS: class object to copy
	OUTPUT: ---
	*/
	OcTreeClass& operator=(OcTreeClass const& other);
	/*
	USAGE: Destructor
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	...
	ARGUMENTS:
	- OcTreeClass& other -> object to swap content from
	OUTPUT: ---
	*/
	void Swap(OcTreeClass& other);
	/*
	...
	*/
	...
	OUTPUT: OcTreeClass object (child in index)
	*/
	OcTreeClass* GetChild(uint a_nChild);
	/*
	USAGE: returns the parent of the octant
	ARGUMENTS: ---
	OUTPUT: OcTreeClass object (parent)
	*/
	OcTreeClass* GetParent(void);
	/*
